...
def create_node(
    node_type: str, node_id: str, node_name: str, **fields: Dict[str, Any]
) -> None:
    """
...
diagnoses = graph_db.get_relations(
    "symptom:loss_of_appetite", "HAS_SYMPTOM", "diagnosis", direction="<-"
)
...
depression_symptoms_result = db.query(
    "SELECT ->HAS_SYMPTOM.* FROM diagnosis:depression"
)
...
for symptom in depression_symptoms:
    ...
    logger.debug(
        f"Symptom: {symptom['id']}. Note: {symptom['note']}. In: {symptom['in']}. Out: {symptom['out']}"
    )
...
for diagnosis in prozac_diagnoses:
    ...
    direction = "<-"
    medications = graph_db.get_relations(
        "medication:prozac", "CONTRAINDICATED_FOR", "medication", direction=direction
    )
