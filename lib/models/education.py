    ...
    def to_dict(self) -> EducationContentType:
        ...
        return {
            ...
            "informationCard": {
                ...
                "features": [{"title": f["title"], "description": f["description"]} for f in self.features]
            },
...
def generate_surrealql_create_query(content: EducationContent, table_name: str = "education") -> str:
    """
...
async def get_education_content_by_id(content_id: str) -> Optional[Dict[str, Any]]:
    ...
    """
    client = await get_client()
    query = f"SELECT * FROM education WHERE id = '{content_id}';"
    result = await client.query(query)
...
async def delete_education_content(content_id: str) -> bool:
    ...
    """
    client = await get_client()
    query = f"DELETE FROM education WHERE id = '{content_id}';"
    result = await client.query(query)
    return len(result) > 0
...
def get_education_schema() -> List[str]:
    ...
    statements.append('DEFINE FIELD informationCard.description ON education TYPE string;')
    statements.append('DEFINE FIELD informationCard.features ON education TYPE array;')
    statements.append('DEFINE FIELD informationCard.features[*].title ON education TYPE string;')
    statements.append('DEFINE FIELD informationCard.features[*].description ON education TYPE string;')
