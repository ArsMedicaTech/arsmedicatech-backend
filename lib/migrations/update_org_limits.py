    ...
    try:
        ...
        for user_data in users_data:
            try:
                ...
                logger.info(
                    f"Updated user {user.username} ({user.id}) organization limit to {new_limit}"
                )
                success_count += 1

            except Exception as e:
                logger.error(
                    f"Failed to update user {user_data.get('username', 'unknown')}: {e}"
                )
                failed_count += 1

        total = success_count + failed_count
        logger.info(
            f"Bulk update complete: {success_count} successful, {failed_count} failed out of {total} total"
        )
...
def get_user_org_info(user_id: Optional[str] = None) -> dict:
    ...
    try:
        if user_id:
            ...
            users_data = (
                result[0].get("result", []) if "result" in result[0] else result
            )
            users_info = []
