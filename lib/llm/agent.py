...
from settings import MCP_TRUSTED_HOSTS, logger
...
async def process_tool_call(
    ...
    for key, val in tool_dict.items():
        logger.debug(
            f"Tool: {key} -> {val}"
        )  # [DEBUG] Tool: _call -> <function fetch_mcp_tool_defs.<locals>.wrap.<locals>._call at 0x7f78720b23e0>
        logger.debug(
            f"Function: {function_name} -> {val.__name__}"
...
class LLMAgent:
    ...
    def __init__(
        ...
        **params: Dict[
            str, str
        ],  # Additional parameters for the agent (e.g., temperature, max_tokens, etc.
    ) -> None:
        """
    ...
    def add_tool(
        self, tool_name: str, tool: Callable[..., Any], tool_def: ToolDefinition
    ) -> None:
        """
    ...
    def from_dict(
        ...
        message_history = data.get(
            "message_history",
            [{"role": "system", "content": "You are a helpful assistant."}],
        )
        if isinstance(message_history, list):
            # Cast to the more flexible type that matches fetch_history return type
            agent.message_history = cast(
                List[Dict[str, Union[str, Sequence[Collection[str]]]]], message_history
            )
        else:
    ...
    async def process_tool_calls(
        ...
        for tool_call in tool_calls:
            ...
            result = await process_tool_call(
                tool_call_cast, self.tool_func_dict, session_id=api_key
            )
        ...
        for result in tool_results:
            self.message_history.append(
                cast(Dict[str, Union[str, Sequence[Collection[str]]]], result)
            )
