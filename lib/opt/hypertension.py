...
def build_hypertension_payload(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    payload: Dict[str, Any] = {
        "meta": {
            "problem_id": "htn-diet-v1",
            "solver": "scipy_slsqp",
            "sense": "minimize",
        },
        "variables": [
            {"name": f"x{i}", "lower": 0, "upper": 10} for i in range(len(df))
        ],
        "parameters": {
        ...
        "objective": {
            "expr": (
                "0.6*dot(sodium_mg,x) + 0.3*dot(saturated_fat_g,x)"
...
def main() -> OptimalSchema:
    ...
    hypertension_schema = OptimalSchema(
        meta=OptimalMetadata(
            problem_id="htn-diet-v1", solver="scipy_slsqp", sense="minimize"
        ),
