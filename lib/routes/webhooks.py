...
def create_webhook_subscription_route() -> Tuple[Response, int]:
    ...
    try:
        ...
        if not all([event_name, target_url, secret]):
            return (
                jsonify(
                    {"error": "Missing required fields: event_name, target_url, secret"}
                ),
                400,
            )
        ...
        if event_name not in valid_events:
            return (
                jsonify(
                    {
                        "error": f"Invalid event_name. Must be one of: {', '.join(valid_events)}"
                    }
                ),
                400,
            )
        ...
        subscription = WebhookSubscription(
            event_name=event_name, target_url=target_url, secret=secret, enabled=enabled
        )
