...
def create_conversation_route() -> Tuple[Response, int]:
    ...
    try:
        success, message, conversation = conversation_service.create_conversation(
            participants, conversation_type
        )

        logger.debug(
            f"Conversation creation result - success: {success}, message: {message}"
        )
        logger.debug(
            f"Conversation object: {conversation.to_dict() if conversation else None}"
        )
...
def send_message_route(conversation_id: str) -> Tuple[Response, int]:
    ...
    try:
        ...
        success, message, msg_obj = conversation_service.add_message(
            conversation_id, current_user_id, message_text
        )
            ...
            for participant_id in conversation.participants:
                if participant_id != current_user_id:
                    ...
                    try:
                        ...
                        sender_name = (
                            sender.get_full_name() if sender else "Unknown User"
                        )
                    finally:
                        user_service.close()
...
def get_user_conversations_route() -> Tuple[Response, int]:
    ...
    try:
        ...
        for conv in conversations:
            logger.debug(
                f"Conversation: {conv.id} - {conv.participants} - {conv.conversation_type}"
            )
        ...
        for conv in conversations:
            ...
            try:
                other_user = (
                    user_service.get_user_by_id(other_participant_id)
                    if other_participant_id
                    else None
                )
                display_name = (
                    other_user.get_full_name() if other_user else "Unknown User"
                )
                avatar = (
                    f"https://ui-avatars.com/api/?name={display_name}&background=random"
                )
            finally:
                user_service.close()
