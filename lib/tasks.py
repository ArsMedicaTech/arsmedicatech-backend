...
def deliver_webhooks(
    event_name: str, payload: Dict[str, Any], max_retries: int = 5
) -> None:
    """
    ...
    try:
        ...
        logger.info(
            f"Delivering webhook for event '{event_name}' to {len(subscriptions)} endpoints"
        )
...
def _deliver_to_subscription(
    ...
    for attempt in range(max_retries + 1):
        try:
            logger.debug(
                f"Attempting webhook delivery to {subscription.target_url} (attempt {attempt + 1})"
            )

            response = requests.post(
                subscription.target_url, data=body, headers=headers, timeout=10
            )
            ...
            logger.warning(
                f"Webhook delivery connection error to {subscription.target_url}"
            )
        except Exception as e:
    ...
    logger.error(
        f"Webhook delivery failed to {subscription.target_url} after {max_retries + 1} attempts"
    )
