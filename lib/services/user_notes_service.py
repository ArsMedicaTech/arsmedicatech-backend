...
class UserNotesService:
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Database controller has connect method: {hasattr(self.db, 'connect')}"
            )
            ...
            if hasattr(self.db, "connect"):
                ...
                logger.debug(
                    "Database controller does not have connect method - using mock mode"
                )
    ...
    def get_user_notes(
        ...
        try:
            logger.debug(
                f"get_user_notes - user_id: {user_id}, include_shared: {include_shared}"
            )
    ...
    def search_notes(
        self, user_id: str, query: str, include_shared: bool = True
    ) -> List[UserNote]:
        """
        ...
        try:
            logger.debug(
                f"search_notes - user_id: {user_id}, query: {query}, include_shared: {include_shared}"
            )
            ...
            logger.debug(
                f"Found {len(notes)} notes matching query '{query}' for user {user_id}"
            )
            return notes
