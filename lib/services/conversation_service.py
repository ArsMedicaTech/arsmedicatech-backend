...
class ConversationService:
    ...
    def create_conversation(
        self, participants: List[str], conversation_type: str = "user_to_user"
        ...
        try:
            ...
            if result:
                ...
                if verification:
                    logger.debug(
                        f"Conversation verification successful: {verification.id}"
                    )
                else:
                    logger.debug(
                        "Conversation verification failed - could not retrieve saved conversation"
                    )
                    ...
                    logger.debug(
                        f"Conversations for participant {participants[0]}: {participant_conversations}"
                    )
    ...
    def get_conversation_by_id(self, conversation_id: str) -> Optional[Conversation]:
        ...
        try:
            ...
            logger.debug(
                f"Looking for conversation with record_id_expr: {record_id_expr}"
            )
