    ...
    def decorator(f: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def decorated_function(*args: Any, **kwargs: Any) -> Any:
            ...
            if not api_key_obj.has_permission(required_permission):
                ...
                return (
                    jsonify({"error": f"Permission '{required_permission}' required"}),
                    403,
                )
    ...
    def decorator(f: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def decorated_function(*args: Any, **kwargs: Any) -> Any:
            ...
            if not has_permission:
                ...
                logger.debug(
                    f"API key missing required permissions ({permission_text}): {required_permissions}"
                )
                return (
                    jsonify(
                        {
                            "error": f"Permissions required ({permission_text}): {', '.join(required_permissions)}"
                        }
                    ),
                    403,
                )
