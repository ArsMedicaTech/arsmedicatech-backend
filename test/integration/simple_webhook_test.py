...
def test_webhook_system():
    ...
    webhook_url = input(
        "Enter your webhook URL (e.g., http://localhost:8000/webhook): "
    ).strip()
    ...
    sub_response = session.post(f"{base_url}/api/webhooks", json=subscription_data)
    ...
    app_response = session.post(f"{base_url}/api/appointments", json=appointment_data)
