...
def test_optimal_service():
    ...
    resp_without_api_key = requests.post(
        "https://optimal.apphosting.services/optimize", json=payload, timeout=10
    )

    print(
        "Response without API key:",
        resp_without_api_key.status_code,
        resp_without_api_key.text,
    )

    assert resp_without_api_key.status_code == 403

    headers = {"x-api-key": TEST_OPTIMAL_KEY}
...
def test_optimal():
    ...
    service = OptimalService(
        url=OPTIMAL_URL, api_key=TEST_OPTIMAL_KEY, schema=hypertension_schema
    )
