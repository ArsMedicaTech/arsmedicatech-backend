...
class WebhookTester:
    ...
    def create_webhook_subscription(
        self, event_name: str, target_url: str, secret: str = "test-secret"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.session.post(f"{self.base_url}/api/webhooks", json=data)
            ...
            if response.status_code == 201:
                ...
                print(
                    f"✅ Created webhook subscription for {event_name}: {subscription.get('id')}"
                )
                return subscription
            else:
                print(
    ...
    def create_appointment(
        self, patient_id: str = "patient:test123", provider_id: str = "provider:test456"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.session.post(f"{self.base_url}/api/appointments", json=data)
            ...
            if response.status_code == 201:
                ...
                print(
                    f"❌ Failed to create appointment: {response.status_code} - {response.text}"
                )
                return {}
    ...
    def update_appointment(self, appointment_id: str) -> bool:
        ...
        try:
            ...
            response = self.session.put(
                f"{self.base_url}/api/appointments/{appointment_id}", json=data
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"❌ Failed to update appointment: {response.status_code} - {response.text}"
                )
                return False
    ...
    def cancel_appointment(self, appointment_id: str) -> bool:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/api/appointments/{appointment_id}/cancel", json=data
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"❌ Failed to cancel appointment: {response.status_code} - {response.text}"
                )
                return False
    ...
    def confirm_appointment(self, appointment_id: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"❌ Failed to confirm appointment: {response.status_code} - {response.text}"
                )
                return False
...
def main():
    ...
    webhook_url = input(
        "Enter your webhook URL (or press Enter for webhook.site): "
    ).strip()
