    ...
    try:
        ...
        if (
            ...
            if (
                ...
                print(
                    f"DB openai_api_key: {db_record.get('openai_api_key', 'None')[:20] if db_record.get('openai_api_key') else 'None'}..."
                )
            ...
            if (
                updated_query
                and isinstance(updated_query, list)
                and len(updated_query) > 0
            ):
                updated_record = updated_query[0]
                updated_encrypted = updated_record.get("openai_api_key", "")
                if updated_encrypted:
                    updated_decrypted = encryption_service.decrypt_api_key(
                        updated_encrypted
                    )
