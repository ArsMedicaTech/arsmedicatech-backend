...
try:
    ...
    print(
        f"✅ Settings imported, ENCRYPTION_KEY generated: {settings.ENCRYPTION_KEY[:10]}..."
    )
...
def test_api_key_saving():
    ...
    try:
        ...
        proper_test_key = (
            "sk-test1234567890123456789012345678901234567890123456789012345678901"[:51]
        )
        ...
        if settings:
            print(
                f"   Current API key in model: {settings.openai_api_key[:20] if settings.openai_api_key else 'None'}..."
            )

            # Set API key directly
            settings.set_openai_api_key(proper_test_key)
            print(
                f"   After setting: {settings.openai_api_key[:20] if settings.openai_api_key else 'None'}..."
            )

            # Test to_dict
            settings_dict = settings.to_dict()
            print(
                f"   to_dict openai_api_key: {settings_dict['openai_api_key'][:20] if settings_dict['openai_api_key'] else 'None'}..."
            )
        ...
        success, message = user_service.update_openai_api_key(
            test_user_id, proper_test_key
        )
        ...
        if success:
            ...
            print(
                f"   Updated settings ID: {updated_settings.id if updated_settings else 'None'}"
            )
            print(
                f"   Has API key after save: {updated_settings.has_openai_api_key() if updated_settings else False}"
            )

            # Get the actual API key
            retrieved_key = user_service.get_openai_api_key(test_user_id)
            print(
                f"   Retrieved key length: {len(retrieved_key) if retrieved_key else 0}"
            )
            ...
            if db_result and isinstance(db_result, list) and len(db_result) > 0:
                ...
                print(
                    f"   DB openai_api_key: {db_settings.get('openai_api_key', 'None')[:20] if db_settings.get('openai_api_key') else 'None'}..."
                )
...
def test_specific_user():
    ...
    specific_user_id = input(
        "Enter a real user ID from your database (or press Enter to skip): "
    ).strip()
