...
def test_api():
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(
                f"Created patient: {created_patient.get('first_name')} {created_patient.get('last_name')}"
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Retrieved patient: {patient.get('first_name')} {patient.get('last_name')}"
                )
            ...
            response = requests.put(
                f"{BASE_URL}/patients/{patient_id}", json=update_data
            )
            print(f"Status: {response.status_code}")
            if response.status_code == 200:
