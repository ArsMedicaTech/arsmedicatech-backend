    ...
    try:
        ...
        for i, result in enumerate(results):
            ...
            try:
                ...
                print(
                    f"    ✅ Parsed: {subscription.id}: {subscription.event_name} -> {subscription.target_url} (enabled: {subscription.enabled})"
                )
        ...
        if results and results[0].get("result") and len(results[0]["result"]) > 0:
            ...
            try:
                ...
                print(
                    f"[direct test] ✅ Parsed: {subscription.id}: {subscription.event_name} -> {subscription.target_url} (enabled: {subscription.enabled})"
                )
            except Exception as e:
