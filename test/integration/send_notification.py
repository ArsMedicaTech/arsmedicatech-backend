...
def send_notification(
    user_id: str, notification_type: str, message: Optional[str] = None
):
    ...
    if notification_type == "new_message":
        ...
        event_data = {
            ...
            "content": message
            or f"Test appointment reminder from CLI at {time.strftime('%H:%M:%S')}",
            "timestamp": str(time.time()),
        }
        ...
        event_data = {
            ...
            "content": message
            or f"Test system notification from CLI at {time.strftime('%H:%M:%S')}",
            "timestamp": str(time.time()),
        }
    else:
        print(f"❌ Unknown notification type: {notification_type}")
        print(
            "   Available types: new_message, appointment_reminder, system_notification"
        )
        return False
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Usage: python send_notification.py <user_id> <notification_type> [message]"
        )
        print("")
        print("Examples:")
        print("  python send_notification.py test-user-123 new_message")
        print(
            "  python send_notification.py test-user-123 new_message 'Hello from CLI!'"
        )
        print(
            "  python send_notification.py test-user-123 appointment_reminder 'Your appointment is tomorrow'"
        )
        print(
            "  python send_notification.py test-user-123 system_notification 'System maintenance scheduled'"
        )
        print("")
